...
function updatePigeon(msg) {
  ...
  if (msg.progress >= 1) { msg.status = "delivered"; msg.deliveredAt = Date.now(); msg.currentLat = msg.receiverLat; msg.currentLng = msg.receiverLng; }
  return msg;
}
...
export class MessengerDO {
  ...
  async webSocketMessage(ws, raw) {
    let msg;
    try { msg = JSON.parse(raw); } catch { return ws.send(JSON.stringify({ type: "error", data: "Invalid JSON" })); }
  ...
  async webSocketClose(ws) {
    ...
    const users = (await this.state.storage.get("users")) || {};
    if (users[att.userId]) { users[att.userId].connected = false; await this.state.storage.put("users", users); }
    this.broadcast({ type: "user_offline", data: { userId: att.userId } });
  }

  async ensureAlarm() {
    if (!(await this.state.storage.getAlarm())) await this.state.storage.setAlarm(Date.now() + TICK_MS);
  }
    ...
    try {
      ...
      for (const msg of messages) {
        if (msg.status === "inTransit") {
          updatePigeon(msg); hasFlying = true;
  ...
  broadcast(msg, excludeId) {
    ...
    for (const ws of this.ctx.getWebSockets()) { const a = ws.deserializeAttachment(); if (a?.userId !== excludeId) try { ws.send(data); } catch {} }
  }

  sendTo(userId, msg) {
    const data = JSON.stringify(msg);
    for (const ws of this.ctx.getWebSockets()) { const a = ws.deserializeAttachment(); if (a?.userId === userId) try { ws.send(data); } catch {} }
  }
}
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === "/") return Response.json({ name: "🕊️ Carrier Pigeon", version: "5.1.0", ws: `wss://${url.host}/ws` });
